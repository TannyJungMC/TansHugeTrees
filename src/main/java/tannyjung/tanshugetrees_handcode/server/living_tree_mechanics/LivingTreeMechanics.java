package tannyjung.tanshugetrees_handcode.server.living_tree_mechanics;
...
import tannyjung.tanshugetrees_handcode.common.config.ConfigMain;
            ...
            if (ConfigMain.leaf_litter == true) {
                ...
                if (Math.random() < ConfigMain.leaf_litter_remover_chance) {
                    ...
                    if (Utils.score.get(level_server, "TANSHUGETREES", "leaf_litter_remover") < ConfigMain.leaf_litter_remover_count_limit) {
                        ...
                        String command = Utils.command.summonEntity("marker", "TANSHUGETREES / TANSHUGETREES-leaf_litter_remover", "Leaf Litter Remover", "ForgeData:{block:\"" + Utils.block.toText(block) + "\"}");
                        Utils.command.run(level_server, pos.getX(), pos.getY(), pos.getZ(), command);
