...
public class DataFolderWorldGenCleaner {
    ...
    public static void start (String dimension, ChunkPos chunk_pos) {
        ...
        File file_region = new File(Core.path_world_data + "/world_gen/#regions/" + file_path);
        ...
        if (FileManager.readBIN(file_region.getPath()).capacity() >= 1024) {
            ...
            new File(Core.path_world_data + "/world_gen/place/" + file_path).delete();
            new File(Core.path_world_data + "/world_gen/detailed_detection/" + file_path).delete();
